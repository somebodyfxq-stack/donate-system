...
import {MultiGoal} from './multiGoalModal';

const widgetCurrencies = widgetEnum.CURRENCIES;

const FirstTab = ({ widget, textPositions, onChange, onSwitchChange, onSaveMultiGoal, onTextEditorChange }) => {
	const [showMultiGoalSettings, setShowMultiGoalSettings] = useState(false);

	// fix to use textAlignments = widgetEnum.TEXT_ALIGNMENT;
	...
	return (
		<>
			...
			<div className="form-group row mb-lg-4">
				<label htmlFor="textPosition" className="col-sm-4 col-form-label">Вертикальна позиція тексту</label>
				<div className="col-sm-4">
					<select id="textPosition" className="form-control"
						value={widget.textPosition}
						onChange={(e) => onChange(e)}>
						{textPositions.map((item, i) =>
			...
			<div className="form-group row mb-lg-4">
				<label htmlFor="textAlignment" className="col-sm-4 col-form-label">
					Горизонтальна позиція тексту</label>
				<div className="col-sm-4">
					<select id="textAlignment" className="form-control"
						value={widget.textAlignment}
						onChange={(e) => onChange(e)}>
						{textAlignments.map((item, i) =>
							<option key={item.id} value={item.id}> {item.name} </option>
						)}
					</select>
				</div>
			...
			<div className="form-group row mb-lg-4">
				<label htmlFor="hideMinMaxNumbers" className="col-sm-4 col-form-label">
					Заховати початкову і кінцеву суми у віджеті
				</label>
				<div className="col-sm-4">
					<Switch id="hideMinMaxNumbers"
						onChange={(checked) => onSwitchChange(checked, 'hideMinMaxNumbers')}
						checked={widget.hideMinMaxNumbers}
					/>
				</div>
			</div>
			...
			<div className="form-group row mb-lg-4">
				<label htmlFor="multiGoal" className="col-sm-4 col-form-label">
					Збір на багато речей?
				</label>
				<div className="col-sm-4">
					<Switch id="multiGoal"
						onChange={(checked) => onSwitchChange(checked, 'multiGoal')}
						checked={widget.multiGoal}
					/>
				</div>
			</div>
			...
			{widget.multiGoal && (
				<div className="form-group row mb-lg-4">
					<label htmlFor="viewType" className="col-sm-4 col-form-label">
						Налаштування мультицілі
					</label>
					<div className="col-sm-5">
						<button className="btn btn-primary mr-2 mr-4"
							onClick={(e) => { e.preventDefault(); setShowMultiGoalSettings(!showMultiGoalSettings) }}>
							Відкрити деталі
						</button>
					</div>
				</div>
			...
			{showMultiGoalSettings && (
				<MultiGoal
					widget={widget}
					show={showMultiGoalSettings}
					handleClose={() => setShowMultiGoalSettings(false)}
					handleConfirm={(data) => { onSaveMultiGoal(data); setShowMultiGoalSettings(false) }}
				/>
			)}
