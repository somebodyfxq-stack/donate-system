...
import {CurrencyDisplay, PaymentSystem, WalletType} from '../../enums/PaymentEnums';
...
const WhitepayPayoutFees = [
	{title: 'Комісія мережі', description: `1 USDT`},
	{title: 'Комісія п.с.', description: formatPercent(PaymentSystem.whitepay.transactionFee)},
	{title: 'Послуги сервісу', description: formatPercent(PaymentSystem.whitepay.serviceFee)}
];

const SliderWithTooltip = createSliderWithTooltip(Slider);

class PayoutsWhitepay extends Component {
	...
	fetchData() {
		this.setState({loading: true}, () => {
			api.getPayouts({system: PaymentSystem.whitepay.name}).then(data => {
				if (data) {
					const prevCard = data.card;
					this.setState({loading: false, prevCard, ...data});
					this.buildPayoutSlider(data.openedPayout?.donates, data.walletType);
	...
	buildPayoutSlider = (payoutDonates, walletType) => {
		...
		const minAmount = walletType === WalletType.card ? PaymentSystem.whitepay.payoutMinAmount : PaymentSystem.whitepay.payoutMinAmount;
		const maxAmount = PaymentSystem.whitepay.payoutMaxAmount;
		const payoutSlider = {};
		let payoutAmount = 0;
		...
		for (const donate of donates) {
			...
			payoutAmount = amount;
			const payoutAmountInt = Math.floor(payoutAmount - PaymentSystem.whitepay.networkFee); // subtract network fee
		...
		const payoutMax = Math.floor(payoutAmount - PaymentSystem.whitepay.networkFee); // subtract network fee
	...
	requestPayout = () => {
		...
		const params = {pubOrderId: donate.pubOrderId, system: PaymentSystem.whitepay.name};
	...
	renderOpenedPayout() {
		...
		const {loading, openedPayout, payoutSlider, selectedPayoutAmount, payoutAllowedTerms, walletType} = this.state;
		const payoutMinAmount = walletType === WalletType.card ? PaymentSystem.whitepay.payoutMinAmountCard : PaymentSystem.whitepay.payoutMinAmount;
		...
		return <div className="section-wrapper">
			...
			{!loading && <>
				{openedPayout?.donates?.length === 0 &&
					<NoDataContainer title="У тебе ще немає нових криптодонатів"
				...
				{openedPayout?.donates?.length > 0 && <div>
					...
					<OpenedPayout
						payoutSystem={PaymentSystem.whitepay.name}
	...
	renderDetails() {
		const {loading, openedPayout, editCard, walletType, card, payoutsHistory, payoutAllowedTerms} = this.state;
		...
		return <div className="row">
			<div className="col-12 col-md-5">
				{<PayoutBalance
					...
					loading={loading}
					terms={WhitepayPayoutTerms}
					fees={WhitepayPayoutFees}
					totalPayoutAmount={openedPayout.totalPayoutAmount}
					payoutMinAmount={PaymentSystem.whitepay.payoutMinAmount}
					payoutMaxAmount={PaymentSystem.whitepay.payoutMaxAmount}
								...
								<div className="input-group" onDoubleClick={this.onEditCard}>
									<div className="input-group-prepend">
										<span className="input-group-text">
											<i className={`${walletType === WalletType.card ? `fa-regular fa-credit-card` : `fa-solid fa-wallet`}`}
											   style={{fontSize: '14px'}}/>
										</span>
									</div>
		...
		return <div>
			...
			<div className="payouts payouts-whitepay">
				...
				{lastPayout && <div className="section-wrapper">
					...
					<PayoutItems payoutSystem={PaymentSystem.whitepay.name}
								 payouts={[lastPayout]}
...
export default connect(mapStateToProps)(PayoutsWhitepay);
